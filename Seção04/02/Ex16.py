def dados_gabarito(gabarito: str, count: int) -> int:
    try:
        for x in range(count):
            ...
            if letra.upper() not in ('A', 'B', 'C', 'D', 'E'):
...
print(gabarito)
for n,listas in enumerate(matriz_final):
    print(f"Aluno: {n+1} {listas}")

for n, x in enumerate(resultado):
    if x >= 7:
        print(f"Aluno:{n+1}, nota: {x}, Aprovado!")
    else:
        print(f"Aluno:{n+1}, nota: {x}, Reprovado!")
